...
A point change model is appropriate as we can observe in the scatter plot 2 "clusters" of points, there are points before t = 37 that appear to be closer around an average and variance, where suddenly for t > 37 there is another cluster following another mean and variance. There also doesnt seem to be a gradual transition, but alot more sharp.
...
sigsq_1s[1] = .Machine$double.eps
sigsq_2s[1] = .Machine$double.eps 
theta_3s[1] = n / 2

for (s in 2 : num_tot_samples){
    cur_theta_1 = theta_1s[s-1]
    cur_sigsq_1 = sigsq_1s[s-1]
    cur_theta_2 = theta_2s[s-1]
    cur_sigsq_2 = sigsq_2s[s-1]
    cur_theta_3 = theta_3s[s-1]
    
    data_1 = x[1:cur_theta_3]
    post_mean = mean(data_1)
    post_var = cur_sigsq_1 / length(data_1)
    theta_1s[s] = rnorm(1, post_mean, sqrt(post_var))
    
    n1 = cur_theta_3
    sse1 = sum((x[1:cur_theta_3] - theta_1s[s])^2)
    alpha0 = 0.001
    beta0 = 0.001 
    alpha = (n1/2) + alpha0
    beta = (sse1/2) + beta0
    precision1 = rgamma(1, shape = alpha, rate = beta)
    sigsq_1s[s] = 1/precision1
    
    data_2 = x[(cur_theta_3+1):n]
    post_mean2 = mean(data_2)
    post_var2 = cur_sigsq_2 / length(data_2)
    theta_2s[s] = rnorm(1, post_mean2, sqrt(post_var2))
    
    n2 = n - cur_theta_3
    sse2 = sum((x[(cur_theta_3+1):n] - theta_2s[s])^2)
    alpha2 = (n2/2) + alpha0
    beta2 = (sse2/2) + beta0
    precision2 = rgamma(1, shape = alpha2, rate = beta2)
    sigsq_2s[s] = 1/precision2
  
    log_likes = numeric(n-1)
    
    for (t3 in 1:(n-1)) {
        seg1 = x[1:t3]
        ll1 = sum(dnorm(seg1, theta_1s[s], sqrt(sigsq_1s[s]), log = TRUE))
  
        seg2 = x[(t3+1):n]
        ll2 = sum(dnorm(seg2, theta_2s[s], sqrt(sigsq_2s[s]), log = TRUE))
        log_likes[t3] = ll1 + ll2
    }
    likes = exp(log_likes - max(log_likes))
    probs = likes / sum(likes)
    theta_3s[s] = sample(1:(n-1), size = 1, prob = probs)
}
```
...
rm(theta_1s, theta_2s, theta_3s, sigsq_1s, sigsq_2s)
```
...
```{r}
gibbs_chain = gibbs_chain[gibbs_chain$t > t_burn_in, ]
```
...
```{r}
num_iid_samples = nrow(gibbs_chain)
print(paste("Number of IID samples after burning and thinning:", num_iid_samples))
```

What would we change in the above code so we could've had more iid samples?

To increase the number of independent samples after burning and thinning, you can increase the total number of samples
...
good but not better than first mean
...
again all landed in the confidence interval.
...
```{r}
theta_1_mmse = mean(gibbs_chain$theta_1)
theta_1_mmae = median(gibbs_chain$theta_1)
```

Find a 95\% CR for theta_2

```{r}
theta_2_cr = quantile(gibbs_chain$theta_2, c(0.025, 0.975))
```

Test H_a: theta_3 < 25.

```{r}
prob_theta_3_less_than_25 = mean(gibbs_chain$theta_3 < 25)
print(paste("Posterior probability that theta_3 < 25:", round(prob_theta_3_less_than_25, 4)))

if(prob_theta_3_less_than_25 > 0.95) {
  print("Strong evidence in favor of H_a: theta_3 < 25")
} else if(prob_theta_3_less_than_25 > 0.9) {
  print("Moderate evidence in favor of H_a: theta_3 < 25")
} else {
  print("Insufficient evidence to support H_a: theta_3 < 25")
}
```
...
Is it possible to run this many permutations? No
...
for (b in 1 : B){
  new_indices = sample(x_ind, n1 + n2, replace = FALSE)
  x_1b = x[new_indices[1:n1]]
  x_2b = x[new_indices[(n1+1):(n1+n2)]]
...
```{r}
p_value = mean(abs(thetahathat_b) >= abs(thetahathat))

hist(thetahathat_b, breaks = 100, main = "Permutation Distribution",
     xlab = "Test Statistic (Difference in Means)", col = "lightblue")
abline(v = thetahathat, col = "red", lwd = 2)
abline(v = -thetahathat, col = "red", lwd = 2, lty = 2)
legend("topright", legend = c("Observed", "Negative of Observed"),
       lty = c(1, 2), col = "red", lwd = 2)
```
...
thetahathat_b = array(NA, B)
for (b in 1 : B){
  thetahathat_b[b] = median(sample(x2, length(x2), replace = TRUE))
}
quantile(thetahathat_b, c(0.025, 0.975))
```

Use the bootstrap to test H_a: theta is nonzero where theta is defined as above.

```{r}
mean(thetahathat_b <= 0)
```
...
thetahathat_b = array(NA, B)
for (b in 1 : B){
  thetahathat_b[b] = quantile(sample(x2, length(x2), replace = TRUE), 0.2)
}
quantile(thetahathat_b, c(0.025, 0.975))
```
...
mle_weibull_iid_compute = function(y){
  k_mle = optimx(par = 1, fn = function(k) {
  -sum(log(k) - k * log(mean(y^k)^(1/k)) + (k-1) * log(y) - (y / mean(y^k)^(1/k))^k)
})$p1
  lambda_mle = 1 / mean(y^k_mle)^(1/k_mle)
...
```{r}
nll_weibull_censored = function(params) {
  k = params[1]
  lambda = params[2]
  ll_observed = sum(log(k) + log(lambda) + (k-1)*log(y_2[c_2 == 0]) - lambda * y_2[c_2 == 0]^k)
  ll_censored = sum(-lambda * y_2[c_2 == 1]^k)
  return(-(ll_observed + ll_censored))
}

result = optimx(par = c(1, 0.001), fn = nll_weibull_censored)
k_mle_censored = result$p1
lambda_mle_censored = result$p2
```
...
```{r}
survfit(Surv(y_2, 1-c_2) ~ 1)
```

Estimate median survival for females.

```{r}
median(survfit(Surv(y_2, 1-c_2) ~ 1))
```

Run the log rank test to attempt to prove male and female survival are different.

```{r}
survdiff(Surv(c(y_1, y_2), 1-c(c_1, c_2)) ~ c(rep(1, length(y_1)), rep(2, length(y_2))))
```
